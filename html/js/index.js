...
function Application() {
    ...
    this.run = function() {
		...
		app.data['office_org'] = (storage.office_org) ? storage.office_org : 'Humboldt-Institut e.V.';
		app.data['office_fn'] = (storage.office_fn) ? storage.office_fn : '';
		app.data['office_ln'] = (storage.office_ln) ? storage.office_ln : '';
		app.data['office_role'] =  (storage.office_role) ? storage.office_role : '';
		app.data['office_addr'] =  (storage.office_addr) ? storage.office_addr : 'Schloss Ratzenried';
		app.data['office_zip'] =  (storage.office_zip) ? storage.office_zip : '88260';
		app.data['office_city'] =  (storage.office_city) ? storage.office_city : 'Argenbühl';
		app.data['office_tel0'] =  (storage.office_tel0) ? storage.office_tel0 : '+49 7522 / 988 - 0';
		app.data['office_email'] =  (storage.office_email) ? storage.office_email : 'info@humboldt-institut.org';
		app.data['office_web'] =  (storage.office_web) ? storage.office_web : 'www.humboldt-institut.org';
		app.update_office_page();
		
		app.data['xmpp_office'] =  (storage.xmpp_office) ? storage.xmpp_office : '';
		app.update_xmpp_office_page();
		...
		if (false) {
			...
			share_btn.onclick = app.click_share_btn;
		}
		else {
			share_btn.classList.add('disabled_btn');
